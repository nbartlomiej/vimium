...
function handleProcessCommand(request, sender) {
  processCommand(request.commandName, request.commandArgs);
}

// // Start action functions
// function prepareCommandModeShortcut(){
//   chrome.tabs.getSelected(null,function(tab) {
//     var currentSite = tab.url;
//     fullCommand = shortcuts[persistentKeyQueue].replace("$activeUrl", currentSite);
//     chrome.tabs.sendRequest(tab.id, {name: "enterCommandModeShortcut", command: fullCommand } );
//   });
// }

var commandArgs= '';
function processCommand(commandName, args){
  commandArgs = args;
  eval("vimium_"+commandName+"();");
}
...
function vimium_open(){
  var url = commandArgs;
  // if the url contains no dot, consider it
  // a query for google search engine
  if (url.indexOf(".")<0){
    url="http://www.google.com/search?q="+url;
  }
  // appending http:// ; if not present, we add it.
...
function vimium_tabnew(){
  var url = commandArgs;
  // if the url contains no dot, consider it
  // a query for google search engin
  if (url.indexOf(".")<0){
    url="http://www.google.com/search?q="+url;
  }
  // appending http:// if not present, we add it.
...
function handleKeyDown(request, port) {
  ...
  if (key == "<ESC>") {
    ...
    persistentKeyQueue = keyQueue + key;
  ...
  if (keyToCommandRegistry[command]) {
    ...
    if (!registryEntry.isBackgroundCommand) {
      ...
      port.postMessage({ command: registryEntry.command,
                         parameter: registryEntry.parameter,
                         frameId: frameId,
                         count: count,
